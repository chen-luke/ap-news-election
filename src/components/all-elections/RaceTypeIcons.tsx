...
const getRaceIcon = (raceType: string) => {
  if (raceType === 'houseRace') return usHouseElectionIcon;
  if (raceType === 'govRace') return usGovernorElectionIcon;
  if (raceType === 'mayorRace') return usMayorElectionIcon;
  if (raceType === 'ballotMeasure') return usBallotMeasureIcon;
  return null;
};
    ...
    <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
      {Object.entries(race).map(([key, value]) => {
        ...
        return (
          Array.isArray(value) &&
          value.map(() => {
            return <div key={uuidv4()}>{getRaceIcon(key)}</div>;
          })
        );
      })}
    </div>
